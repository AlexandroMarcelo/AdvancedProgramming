...
void filterTheImage(ppm_t * original_image, filter_image_t * filter)
{
    ...
    int status; //if it fails
    int height_jump = original_image->height/NUM_THREADS;
    int start_height_jump = 0;
    int end_height_jump = 0;
    thread_data_t dataThread[NUM_THREADS]; //creating the data for threads
    int distance = filter->height / 2; //for ignoring the edges

    for (int i = 0; i < NUM_THREADS; i++)
    {
        //Saving the data to thre structure for each thread
...
void * filterTheImageWithThreads(void * arg)
{
    ...
    double new_value_G;
    double new_value_B;
    int cont = 0; //for carrying on a variable for each thread for the rows
...
//#pragma omp parallel for default(none) shared(distance, original_image->pixels) private(new_value_R, new_value_G, new_value_B, filtered_image)
	for (int rows_image = thread_data->distance+thread_data->start_height; rows_image < thread_data->end_height-thread_data->distance; rows_image++)
	{
		...
		{
            //setting the variables to 0 each time
            new_value_R = 0;
            new_value_G = 0;
            new_value_B = 0;
            ...
            //if the new value overpass the limits
            if (new_value_R > 255){
                new_value_R = 255;
            }
            if (new_value_G > 255){
    ...
    cont = 0;
    //Storing the new data of the filtered image
...
void copyStructureData(ppm_t * original_image, ppm_t * filtered_image, int start_height, int end_height)
{
    ...
    //copy each pixel to the other structure
